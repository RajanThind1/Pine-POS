[README-2.md](https://github.com/user-attachments/files/29647973/README-2.md)
...
| **Guest** | `/guest?table=N` | The diner's phone — QR code at the table |
...
- Refund a transaction in the back office and watch the numbers move
  everywhere.
- Open the **Guest app** (`/guest?table=3`) in a fourth window — the diner's
  view. Tap **Request the Check** and a toast lands on the handheld and POS
  the same second; tap **Pay & Go**, add a tip, pay — every staff screen
  announces *"Table 3 paid $X via Pay & Go — table is free"*, the floor plan
  flips the table to needs-bussing, and the payment drops straight into the
...
### Guest (`/guest?table=N`)
What the QR code at each seat opens — no login, no app install. Diners see
their **live check** (it updates as the server rings items), can **Request
the Check** (flags the table amber and notifies staff instantly), or use
**Pay & Go**: pick a tip, pay on a simulated card, and walk out. The
transaction records like any card payment (`payAndGo: true`, unsettled →
open batch), the table flips to needs-bussing, and every staff screen gets
a real-time toast with the table number — built for the Friday-night rush
when servers can't get to the card reader fast enough.
